...
def course(course_num):
    ...
    available = Session.query.filter(Session.course.has(course_num=course_num)).all()
    ...
    if request.method == "POST":
        if is_valid(form.filename.data.filename):
            ...
            setting = Session.query.filter(Session.course.has(course_num=course_num)).filter_by(session_num=form.sessions.data).first()
...
def add_course():
    ...
    if request.method == "POST":
        ...
        if Course.query.filter_by(course_num=form.course_num.data).first() is None:
            new_course = Course(course_num=form.course_num.data, registration=form.registration_link.data)
